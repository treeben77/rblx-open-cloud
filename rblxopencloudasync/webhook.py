...
class Webhook():
    ...
    async def process_notification(self, body: bytes, secret_header: bytes = None, validate_signature=True) -> tuple[str, int]:
        """
        ...
        try:
            ...
            await function(notification=notification)
        except(Exception) as error:
            if self.__on_error:
                await self.__on_error(notification, error)
            else:
                raise error
