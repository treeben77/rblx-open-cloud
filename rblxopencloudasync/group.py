...
class Group(Creator):
    ...
    async def fetch_info(self) -> "Group":
        ...
        status, data, headers = await preform_request("GET", f"cloud/v2/groups/{self.id}", authorization=self.__api_key)

        if status == 401: raise InvalidKey(data)
    ...
    async def list_members(self, limit: Optional[int]=None, role_id:Optional[int] = None, user_id: Optional[int] = None) -> AsyncIterator["GroupMember"]:
        """
        ...
        while limit == None or yields < limit:
            status, data, headers = await preform_request("GET", f"cloud/v2/groups/{self.id}/memberships", authorization=self.__api_key, params={
    ...
    async def list_roles(self, limit: Optional[int]=None) -> AsyncIterator[GroupRole]:
        """
        ...
        yields = 0
        while limit == None or yields < limit:
            status, data, headers = await preform_request("GET", f"cloud/v2/groups/{self.id}/roles", authorization=self.__api_key, params={
    ...
    async def fetch_shout(self) -> "GroupShout":
        ...
        status, data, headers = await preform_request("GET", f"cloud/v2/groups/{self.id}/shout", authorization=self.__api_key)
