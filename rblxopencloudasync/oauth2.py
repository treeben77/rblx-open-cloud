...
class PartialAccessToken():
    ...
    async def fetch_userinfo(self) -> User:
        ...
        status, data, headers = await preform_request("GET", "oauth/v1/userinfo", authorization=f"Bearer {self.token}")
        
        user = User(data.get("id") or data.get("sub"), f"Bearer {self.token}")
        user.username: str = data.get("preferred_username")
        user.display_name: str = data.get("nickname")
        user.headshot_uri: str = data.get("picture")
        user.created_at: datetime.datetime = datetime.datetime.fromtimestamp(data["created_at"]) if data.get("created_at") else None

        if 200 <= status < 300: return user
        elif status == 401:
            if data["error"] == "insufficient_scope":
                raise InsufficientScope(data["scope"], f"The access token does not have the required scope:'{data['scope']}'")
    ...
    async def fetch_resources(self) -> Resources:
        ...
        status, data, headers = await preform_request("POST", "oauth/v1/token/resources", data={
            "token": self.token,
    ...
    async def fetch_token_info(self) -> AccessTokenInfo:
        ...
        status, data, headers = await preform_request("POST", "oauth/v1/token/introspect", data={
            "token": self.token,
            "client_id": self.app.id,
            "client_secret": self.app._OAuth2App__secret
        })
        
        if 200 <= status < 300: return AccessTokenInfo(data)
        elif status == 401:
            if data["error"] == "insufficient_scope":
                raise InsufficientScope(data["scope"], f"The access token does not have the required scope:'{data['scope']}'")
            raise InvalidKey("The key has expired, been revoked or is invalid.")
    ...
    async def revoke(self):
        await self.app.revoke_token(self.token)
...
class AccessToken(PartialAccessToken):
    ...
    async def revoke_refresh_token(self):
        ...
        """
        await self.app.revoke_token(self.refresh_token)
...
class OAuth2App():
    ...
    async def exchange_code(self, code: str, code_verifier: Optional[str]=None) -> AccessToken:
        """
        ...
        """
        status, data, headers = await preform_request("POST", "oauth/v1/token", data={
            "client_id": self.id,
        ...
        if data.get("id_token"):
            if not self.__openid_certs_cache or time.time() - self.__openid_certs_cache_updated > self.openid_certs_cache_seconds:
                c_status, c_data, _ = await preform_request("GET", "oauth/v1/certs")

                if not 200 <= c_status < 300: raise ServiceUnavailable("Failed to retrieve OpenID certs.")

                self.__openid_certs_cache = []
                for cert in c_data["keys"]:
            ...
            for cert in self.__openid_certs_cache:
                try:
                    id_token = jwt.decode(data["id_token"], cert,  algorithms=['ES256'], audience=str(self.id))
                    break
        ...
        if 200 <= status < 300: return AccessToken(self, data, id_token)
        elif status == 400: raise InvalidKey(data.get("error_description", "The client id, client secret, or redirect uri is invalid."))
        elif status == 401: raise InvalidCode(data.get("error_description", "The code is invalid, or has been used."))
        elif status >= 500: raise ServiceUnavailable("The service is unavailable or has encountered an error.")
        else: raise rblx_opencloudException(f"Unexpected HTTP {status}")
    ...
    async def refresh_token(self, refresh_token: str) -> AccessToken:
        ...
        if 200 <= status < 300: return AccessToken(self, data, None)
        elif status == 400: raise InvalidKey(data.get("error_description", "The code, client id, client secret, or redirect uri is invalid."))
        elif status >= 500: raise ServiceUnavailable("The service is unavailable or has encountered an error.")
        else: raise rblx_opencloudException(f"Unexpected HTTP {status}")
    ...
    async def revoke_token(self, token: str):
        ...
        status, data, headers = await preform_request("POST", "oauth/v1/token/revoke", data={
            "token": token,
            "client_id": self.id,
            "client_secret": self.__secret
        })
        ...
        elif status == 400: raise InvalidKey("The code, client id, client secret, or redirect uri is invalid.")
        elif status >= 500: raise ServiceUnavailable("The service is unavailable or has encountered an error.")
        else: raise rblx_opencloudException(f"Unexpected HTTP {status}")
